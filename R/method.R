...
# Spark version of make.method. Should only refer to method implemented using Spark
...
# Spark version of check.method
check.method.spark <- function(method, data, where, blocks, defaultMethod) {
  if (is.null(method)) {
    print("**DEBUG** method is null, make and return default method")
    return(make.method.spark(
      data = data,
      where = where,
      blocks = blocks,
      defaultMethod = defaultMethod
